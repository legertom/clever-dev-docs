          ...
          <section id="design-decisions" className="scroll-mt-6">
            ...
            <dl className="space-y-5">
              ...
              <DecisionItem
                ...
                definition="Given that one database (above), two ways to query it — this decision is about retrieval quality, not infrastructure. Vector ranks chunks purely by embedding cosine similarity — cheaper, faster, strong on paraphrased questions, but it underweights exact tokens buried in code or JSON. Hybrid adds a parallel full-text search fused via Reciprocal Rank Fusion, rescuing keyword hits the embedding missed (a question like “is the created date shared by default?” returns nothing under vector and five chunks under hybrid). That recall isn’t free: the rescued chunks make prompts longer, so hybrid costs modestly more per query and runs slightly slower. In the eval set hybrid consistently hallucinates less — the failure mode that matters most for a docs assistant — while correctness is comparable and varies run to run; on already-strong models, vector can edge it on cost and correctness. Hybrid is the default for the lower hallucination rate, but it’s a tradeoff, not a clean win — the toggle on the chat and eval pages runs either, or both side by side, so you can judge it on the live numbers."
              />
              <DecisionItem
