...
**6. Multi-turn query rewriting before classification and retrieval.** A bare follow-up like "what about student email?" makes sense reading the conversation but means nothing to a retrieval system in isolation — and reads as off-topic to the pre-flight classifier, which would bounce it before retrieval ran. A cheap `gpt-4o-mini` call condenses the conversation into a single self-contained question ("what student email fields does Clever Library expose?") before *both* steps run. The classifier judges the expanded form, and retrieval embeds and searches on it. Single-turn conversations skip the rewrite entirely (no history to condense), so the existing single-turn eval set sees zero change. Errors fall back to the raw last message so a rewrite failure never breaks the chat. Toggleable via `QUERY_REWRITE=off`. Without this, multi-turn conversations were the system's biggest failure mode: a developer asking three good follow-up questions got one decent answer and two "I can only answer questions about the Clever developer platform" deflections — exactly what made the assistant feel brittle.
